...
namespace PartsRegistration.Controllers
{
    public class AJAXController : Controller
    {
        ...
        public const int testUser = 39; //Тестовый вход под любым пользователем//598 181 211 783 Мазанов - 471 37 Капацин Антон-174 39
        ...
        {
            var allAnswersByQuestion = db2.TestsItem.Where(w => w.TestsId == id && w.QuestionId == idQuestion).ToList();
            ...
            var ti = db2.TestsItem.SingleOrDefault(s => s.TestsId == id && s.AnswerId == idAnswer);
            ti.Selected = idAnswer;
            db2.Entry(ti).State = EntityState.Modified;
            db2.SaveChanges();

            var test = db2.Tests.SingleOrDefault(s => s.TestId == id);
            test.Progress = index;
            db2.Entry(test).State = EntityState.Modified;
            db2.SaveChanges();
        ...
        {
            var ti = db2.Tests.SingleOrDefault(s => s.TestId == id);
        ...
        {
            int? fin = db2.Tests.Find(id).Finished;
        ...
        [HttpPost]
        public int addCategory(int id, int userId, string name)
        {
        ...
        public JsonResult GetUserMark(int id = 0)
        {
            ...
            if (id != 0)
            {
                ...
                var usersmarks = attestation.empItems;
                ...
                {
                    var testsList = db.Tests.Where(w => w.UserId == user.UserID && w.Finished == 1 && w.AttestationId == attestation.AttestationId).ToList();
