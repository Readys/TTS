    ...
    public class TTSController : Controller
    {
        ...
        public ActionResult Index(int page = 1)
        {
            ...
            if (access.Any())
            {
                ...
                {
                    testTemplates = testTemplates.Where(x => x.CreatedByUserId == appUser.UserId).ToList();
                }
            }
            else
            {
        ...
        public ActionResult DisplayAtt(int id)
        {
            ...
            var access = db2.UserInRole.Where(x => x.UserId == appUser.UserId).ToList();

            #endregion

            var thisAtt = db2.Attestations.SingleOrDefault(s => s.AttestationId == id);
            var tests = db2.Tests.Where(w => w.AttestationId == id && w.UserId == appUser.UserId).ToList();

            var thisTestTemplate = db2.TestTemplate.SingleOrDefault(s => s.TestTemplateId == thisAtt.TestTemplateId);
            var threshold = thisTestTemplate.TargetProcent;

            var thisUserAtt = thisAtt.empItems.SingleOrDefault(s => s.SUserId == appUser.SUserId);
        ...
        public ActionResult CreateCertification(int id = 0)
        {
            ...
            Attestation attest;
            var allUsers = h_db.User.Where(s => s.AccountAD != null && s.Fired == false).ToArray();
            var appUser = h_db.User.FirstOrDefault(x => x.UserId == testUser);
            ...
            if (appUser != null && access.Any())
            {
                ...
                {
                    var manager = h_db.User.FirstOrDefault(x => x.UserId == request.LMUserId);
        ...
        public ActionResult SubmitCertification(CreatedCertificationData data)
        {
            ...
            if (testUser == 0)
            {
                appUser = allUsers.FirstOrDefault(x => x.AccountAD.ToUpper() == memberAD.ToUpper());
            }
        ...
        {
            var allUsers = h_db.User.Where(x => x.AccountAD != null && x.Fired != true).ToArray();
            var sevenSamurais = db.UserInRole.Where(x => x.AccessLevelId == 2).Select(s => s.UserId).ToArray();
            ...
            string StartEndDate = null, string FinishEndDate = null)
        {
            ...
            var AllCertifications = db.Attestations;
            ...
            if (appUser != null && isHasAccess)
            {
                ...
                if (SuperAdmin.UserId != appUser.UserId)
                {
                    ...
                    if (oneOfLMUsers && !isOneOfSevenGods)
                    {
                ...
                if (CreatedBy != 0)
                {
                    ...
                    var attestObj = h_db.User.FirstOrDefault(f => f.UserId == CreatedBy).NameRu;
                    if (attestObj != null)
                    {
                ...
                if (ManagerId != 0)
                {
                    ...
                    var attestObj = h_db.User.FirstOrDefault(f => f.UserId == ManagerId);
                    if (attestObj != null)
                    {
        ...
        public ActionResult PassTest(int testTemplateId, int attestationId, int testId = 0)
        {
            ...
            var thisTemplate = db2.TestTemplate.SingleOrDefault(s => s.TestTemplateId == testTemplateId);
            var thisTest = db2.Tests.Any(a => a.AttestationId == attestationId && a.TemplateId == testTemplateId && a.UserId == appUser.UserId && a.Finished == 0);

            if (thisTest == true)
            {
                testId = db2.Tests.SingleOrDefault(a => a.AttestationId == attestationId && a.TemplateId == testTemplateId && a.UserId == appUser.UserId && a.Finished == 0).TestId;
            }

            var maxCount = db2.TestTemplate.SingleOrDefault(s => s.TestTemplateId == testTemplateId).TryCount;
            var tryCount = db2.Tests.Where(w => w.AttestationId == attestationId && w.UserId == appUser.UserId && w.Finished == 1).Count();
            ...
            var checkFinished = db2.Tests.SingleOrDefault(s => s.TestId == testId);
            ...
            var time = db2.TestTemplate.SingleOrDefault(s => s.TestTemplateId == testTemplateId).Duration;

            DateTime startTime = new DateTime();

            startTime = db2.Tests.SingleOrDefault(s => s.TestId == testId).CreateDate.Value;
            ...
            var testTemplateName = db2.TestTemplate.SingleOrDefault(s => s.TestTemplateId == testTemplateId).NameRu;

            var subjectsId = db2.TestTemplateItem.Where(w => w.TestTemplateId == testTemplateId && w.CategoryType == 1).Select(s => s.CategoryId).ToList();
            var subjects = tree.Where(w => subjectsId.Contains(w.TreeId)).ToList();

            var levelsId = db2.TestTemplateItem.Where(w => w.TestTemplateId == testTemplateId && w.CategoryType == 3).Select(s => s.CategoryId).ToList();
            var levels = tree.Where(w => levelsId.Contains(w.TreeId)).ToList();

            var allQuestions = db2.Questions.Where(w => w.Deleted != 1 && w.Published == 1).ToList();
            ...
            var questionsIdArr = db2.TestsItem.Where(w => w.TestsId == testId).Select(s => s.QuestionId).Distinct().ToList();
            var questions = Questions.Where(w => questionsIdArr.Contains(w.QuestionId)).ToList();

            var thisTestsItems = db2.TestsItem.Where(w => w.TestsId == testId).ToList();

            var answers = db2.Answers.ToList();
            var type = db2.Tree.Where(w => w.TypeId == 4).ToList();
            var uploads = db2.Uploads.ToList();
            ...
            foreach (var q in questions)
            {
                ...
                var selectedAnswerId = db2.TestsItem.Where(w => w.QuestionId == q.QuestionId && w.Selected != null && w.TestsId == testId).Select(s => s.AnswerId).ToArray();
                ...
                var questionSubjectsId = db2.TreeQuestion.Where(w => w.QuestionId == q.QuestionId && w.TypeId == 1).Select(s => s.TreeId).ToArray();
                ...
                foreach (var sub in subQuestions)
                {
                    ...
                    var subAnswerItemSelected = db2.TestsItem.SingleOrDefault(s => s.AnswerId == answerSelected.AnswerId && s.TestsId == testId && s.Answers == null); 
            ...
            ViewBag.Progress = db2.Tests.SingleOrDefault(s => s.TestId == testId).Progress;
            ViewBag.Finished = db2.Tests.SingleOrDefault(s => s.TestId == testId).Finished;
        ...
        {
            var allUsers = h_db.User.Where(x => x.AccountAD != null).ToList();
            var allUsersOut = db2.L_User.Where(x => x.PrivateEmail != null).ToList();

            var appUser = Helper.GetAppUser(User.Identity);

            var thisTest = db2.Tests.SingleOrDefault(s => s.TestId == testId);
            var thisUser = allUsersOut.SingleOrDefault(s => s.SUserId == thisTest.SUserId);
            var thisAttestation = db2.Attestations.Find(thisTest.AttestationId);

            var thisTemplate = db2.TestTemplate.FirstOrDefault(s => s.TestTemplateId == thisAttestation.TestTemplateId);

            var tree = db2.Tree.Where(w => w.TypeId == 1 && w.Deleted != 1).ToList();
            var categoriesId = db2.TestTemplateItem.Where(s => s.TestTemplateId == thisTemplate.TestTemplateId && s.CategoryType == 1).Select(s => s.CategoryId).ToArray();
            ...
            var questionIdArr = db2.TestsItem.Where(w => w.TestsId == testId && w.Answers == null).Select(s => s.QuestionId).ToArray();
            questionIdArr = questionIdArr.Distinct().ToArray();
            var allQuestions = db2.Questions.Where(w => w.Deleted != 1 && questionIdArr.Contains(w.QuestionId)).ToList();
            ...
            var testsItems = db2.TestsItem.Where(w => w.TestsId == testId).ToList();
            var rightWeight = 0;

            var thisTargetPercent = db2.TestTemplate.Find(thisAttestation.TestTemplateId).TargetProcent;

            var id = thisTest.TemplateId;

            var maxCount = db2.TestTemplate.SingleOrDefault(s => s.TestTemplateId == id).TryCount;
            var tryCount = db2.Tests.Where(w => w.TemplateId == id && w.SUserId == appUser.SUserId && w.Finished == 1).Count();
            var tryCountNow = maxCount - tryCount;

            var maxWeight = db2.TestTemplate.SingleOrDefault(s => s.TestTemplateId == id).TargetProcent;
           ...
           int? totalTestSumm = 0;
            ...
            var thisEmployeeTestMark = thisAttestation.empItems.FirstOrDefault(f => f.SUserId == thisTest.SUserId.Trim());
            ...
            db2.Entry(thisTest).State = EntityState.Modified;
            db2.SaveChanges();
        ...
        public int GenerateMyTest(int testTemplateId, int attestationId)
        {
            ...
            var tree = db2.Tree.Where(w => w.Deleted != 1).ToList();
            var levelId = db2.TestTemplate.SingleOrDefault(s => s.TestTemplateId == testTemplateId).LevelId;

            var subjectsId = db2.TestTemplateItem.Where(w => w.TestTemplateId == testTemplateId && w.CategoryType == 1).Select(s => s.CategoryId).ToList();
            var subjects = tree.Where(w => subjectsId.Contains(w.TreeId)).ToList();
            var maxDuration = db2.TestTemplate.SingleOrDefault(s => s.TestTemplateId == testTemplateId).Duration;

            var allSubQwestion = db2.Questions.Where(w => w.Deleted != 1 && w.ParentQuestionId != null).ToList();
            var allAnswers = db2.Answers.Where(w => w.Deleted != 1).ToList();
            ...
            {
                var questionQuantity = db2.TestTemplateItem.SingleOrDefault(s => s.CategoryId == sub.TreeId && s.TestTemplateId == testTemplateId).QuestionQuantity;
                var questionIdArr = db2.TreeQuestion.Where(w => w.TreeId == sub.TreeId).Select(s => s.QuestionId).ToArray();
                var questions = db2.Questions.Where(w => w.Deleted != 1 && w.Published == 1 && w.ParentQuestionId == null && questionIdArr.Contains(w.QuestionId)).ToList();
                int subjectLehgth = db2.TreeQuestion.Where(w => w.TreeId == sub.TreeId).Count();
        ...
        public JsonResult GetUser(string search, int id = 0)
        {
            ...
            {
                var empArr = h_db.User.Where(s => s.Fired == false && s.NameRu.ToLower().Contains(search.ToLower())).ToList();
            ...
            if(id != 0)
            {
                var emp = h_db.User.Where(s => s.Fired != true && s.UserId == id).Select(s=> new { Id = s.UserId, Name = s.NameRu });
                return Json(new { emp }, JsonRequestBehavior.AllowGet);
            }
        ...
        public JsonResult AddNewEmployeeItemToCertification(int UserId, int attestId)
        {
            ...
            var employee = h_db.User.FirstOrDefault(s => s.UserId == UserId);
        ...
        public JsonResult AddAllLMEmployees(int LMId, int attestId)
        {
            ...
            AttestationEmployeeItems item = new AttestationEmployeeItems();
            var attestation = db.Attestations.Find(attestId);
            var LmName = h_db.User.FirstOrDefault(f => f.UserId == LMId).NameRu;
            var employees = h_db.User.Where(s => s.LineManagerUserId == LMId && s.Fired != true).ToArray();
        ...
        public JsonResult GetCertificationCreator(string search, int tabs)
        {
            if (!String.IsNullOrEmpty(search))
            {
                ...
                if (tabs == 0)
                {
                    ...
                    var creators = h_db.User.Where(w => certifications.Contains(w.UserId)).Select(s=> new { Id = s.UserId, Name = s.NameRu }).ToArray();
                ...
                if (tabs == 2)
                {
                    ...
                    var creators = h_db.User.Where(w => certifications.Contains(w.UserId)).Select(s => new { CreatorId = s.UserId, Name = s.NameRu }).ToArray();
                    var creatorArr = creators.Where(w => w.Name.ToLower().Contains(search.ToLower())).ToArray();
        ...
        public JsonResult GetSelectedManager(int managerId)
        {
            if (managerId != 0)
            {
                var temp = h_db.User.FirstOrDefault(f => f.UserId == managerId);
                var t = new t();
                t.Id = temp.UserId;
        ...
        {
            var questions = db2.Questions.Where(w => w.Deleted != 1).ToList();

            var uploads = db2.Uploads.ToList();
            var attestation = db2.Attestations.SingleOrDefault(w => w.AttestationId == CertificationId);
            var catTree = db2.Tree.SingleOrDefault(s => s.TreeId == categoryId);

            var actualTestsItems = db2.TestsItem.Where(w => w.TestsId == testid).ToList();
            ...
            var actualTextQuestions = db2.Questions.Where(w => actualTestsItemsTextQuestionsId.Contains(w.QuestionId)).ToList();
            var answers = db2.Answers.Where(w => actualTestsItemsQuestionsId.Contains(w.QuestionId)).ToList();

            var bestTest = db2.Tests.SingleOrDefault(s => s.TestId == testid);//attestationTests.Where(w => w.UserId == user.UserId && w.Finished == 1).OrderByDescending(o => o.WeightResult).First();
